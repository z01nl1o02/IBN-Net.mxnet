...
class RESNET_IBN_A(gluon.nn.HybridBlock):
    def __init__(self,units, num_stage, filter_list, num_class, data_type, ibn, bottle_neck=True, bn_mom=0.9, workspace=512, memonger=False, **kwargs):
        super(RESNET_IBN_A, self).__init__(**kwargs)
        self.net = gluon.nn.HybridSequential()
        with self.name_scope():
            sym = resnet( units, num_stage, filter_list, num_class, data_type, ibn, bottle_neck, bn_mom, workspace, memonger)
            internals = sym.get_internals()
            #for key in sym.get_internals().list_outputs():
            #   print key
            input = internals['data']
            output = internals['resnet_ibn_a0_fc1_output']
            self.net.add( gluon.nn.SymbolBlock(outputs = output, inputs = input) )
        return
...
if 0:
    ...
    net = RESNET_IBN_A(units = units, num_stage=3, filter_list = filter_list, num_class=num_classes, data_type='cifar10',
                       ibn=True, bottle_neck = bottle_neck, bn_mom=0.9, workspace=512,memonger = False)
    ctx = mx.gpu()
    net.initialize( ctx = ctx )
    print net(mx.nd.ones((23,3,32,32), ctx = ctx))